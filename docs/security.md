...
- MCP uses one dedicated stage-specific Service Auth token on its own hostname. Pairing is only available to an authenticated user on the separately protected admin hostname and is not exposed as MCP tools.
...
- Attachments are private R2 objects and are served only through authenticated application paths with `no-store`, `nosniff`, and download disposition headers.
